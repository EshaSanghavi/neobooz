...
class ChattingController extends Controller
{
    ...
    public function chat_list(Request $request, $type)
    {
        ...
        // } else if ($type == 'admin') {
            ...
            $lastChatting = Chatting::with('admin')->where('user_id', auth('customer')->id())
                ...
                ->first();
            if (isset($lastChatting)) {
                $lastChatting = Chatting::with('admin')->where('admin_id', 1)
                    ->orderBy('created_at', 'DESC')
                    ->first();
                Chatting::where(['user_id' => auth('customer')->id(), 'admin_id' => 1])->update(['seen_by_customer' => 1]);

                $chattings = Chatting::join('admins', 'admins.id', '=', 1)
                ...
                $Admins = Chatting::join('admins', 'admins.id', '=', 1)
    ...
    public function messages(Request $request)
    {
        ...
        {
            Chatting::where(['user_id'=>auth('customer')->id(), 'admin_id'=> 1])->update([
                'seen_by_customer' => 1
            ]);

            $shops = Chatting::join('admins', 'admins.id', '=', 'chattings.admin_id')
                ->select('chattings.*', 'admins.name', 'admins.image')
                ->where('user_id', auth('customer')->id())
                ->where('chattings.admin_id', 1)
                ->orderBy('created_at', 'ASC')
                ->get();
        }
